...
md"# Using bayesian inference with Turing"
...
# Import dataset
raw_data = CSV.File("../data/joint_dataset_4analysis.csv"; missingstring="NA", pool=true) |> DataFrame
...
# ╔═╡ 7dc22de4-cf99-4d57-91a9-b7cb64a59ece


# ╔═╡ eadf6acc-7cad-4c61-baab-58a286db7fed
# create log OD response variable
data.logOD = log.(10, 1 .+ data.OD)

# ╔═╡ 101e5ac9-d6fb-43b4-9195-8d831be8766e
# create iswild
data.iswild = data[:,:islab] .- 1 # dataset only had "islab", so -1 and squaring gives the opposite

# ╔═╡ 162de178-8d8f-45a2-af77-325eecc7404b
data.iswild = data[:,:iswild].^2

# ╔═╡ 61aa02b2-94c1-49c9-ab74-62fa1ef23fc1
# create islow
data.islow = data[:,:ishigh] .- 1 # as above

# ╔═╡ 1ba1a68c-e172-4717-9dc2-7e5afc7f816f
data.islow = data[:,:islow].^2

# ╔═╡ 5f74ddc0-814d-40a1-b7c0-2bf3c173d47a
# Specify how to correctly treat columns
autotype(data)

# ╔═╡ 3f260b4e-c451-4981-a138-481e730f71a6
...
# ╔═╡ 948d5ac4-39fa-4125-8fb8-867800e8ae3a
Text(precis(data; io=String))

# ╔═╡ 8800062f-9941-4965-9b34-2fd83cfbf679
md"## Construct a Structural Causal Model"

# ╔═╡ 370c8b64-489e-4b3e-a7af-3cb14aabcd89
#= from Dagitty.net:
dag {
bb="-2.988,-4.231,3.169,5.229"
Diet [exposure,pos="-2.142,2.420"]
Env [pos="-2.475,-0.890"]
Fat_Scores_Sum [pos="0.325,-1.984"]
OD [outcome,pos="0.380,1.193"]
Sex [pos="2.656,0.178"]
Treatment [exposure,pos="2.458,-1.432"]
Weight [pos="-0.431,-0.533"]
days_since_1st_D_inj [pos="1.306,2.604"]
repro [pos="-2.057,-3.442"]
Diet -> OD
Diet -> Weight
Env -> Fat_Scores_Sum
Env -> OD
...
Sex -> OD
Sex -> Weight
Treatment -> Fat_Scores_Sum
Treatment -> OD
Treatment -> Weight
Weight -> OD
days_since_1st_D_inj -> OD
repro -> Fat_Scores_Sum
repro -> Weight
}
=#
...
# ╔═╡ da2015d3-e983-41fe-a822-190a533dab57
@model function vacc()
	OD
...
git-tree-sha1 = "33240ee1f21bb569c3656a84595ff9d31c3e79d5"
