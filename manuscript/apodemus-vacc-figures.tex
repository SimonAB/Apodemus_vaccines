...
\newcommand{\FigDAGtikz}{
    \begin{figure}
        ...
        \caption{\textbf{Graphical Causal Model of vaccine efficacy.} \textbf{A}, Directed acyclic graph representing hypothesised causal effects expressed by the SCM \(\mathfrak{C}\) (Model~\ref{eq:SCM}, right) driving variation in efficacy \(E\) of vaccine \(V\) in laboratory or wild mice \(H\), under supplemented or control diet \(D\) (green vertices depict those that were experimentally intervened upon). Covariates and potential confounders (blue vertices) were time since vaccination \(T\), parasite infection (presence or burden) \(P\), reproductive status \(R\), body mass \(M\), fat scores \(F\), and sex \(S\). Directed edges depict causal effects hypothesised to operate between vertices. \textbf{B}, Assignments derived from the DAG with coefficients $\beta_{ij}$ representing the effect of cause $i$ on effect $j$. $\mathcal{U}_j$ represents the unmeasured variation of the response variable, not represented in the DAG panel A for clarity.}
...
\newcommand{\FigDAG}{
    \begin{figure}
        ...
        \caption{\textbf{Graphical Causal Model of vaccine efficacy.} Directed acyclic graph representing validated causal relationships expressed by the SCM \(\mathfrak{C}\) (Model~\ref{eq:SCM}). Covariates and potential confounders (blue vertices) were time since vaccination \(T\), parasite infection (presence or burden) \(P\), reproductive status \(R\), body mass \(M\), fat scores \(F\), and sex \(S\). }
        \label{fig:DAG}
    \end{figure}
}
...
%                 \includegraphics[width=\hsize]{./Figures/Apodemus-vacc-figures/IgG1.png}
%                 \caption{\textbf{Vaccine-induced DTV-specific IgG1.}}
%                 \todo[inline]{Fix figure \& add legend}
%                 \label{fig:igg1}
%     \end{wrapfigure}
% }
...
\newcommand{\FigIgG}{
    \begin{figure}[hp]
        ...
        \caption{\textbf{Vaccine-induced DTV-specific IgG1.} \textbf{A}, diphtheria toxoid vaccine (DTV)-specific IgG1 optical density in mice from laboratory- and wild-habitats offered high and low quality dietary supplements. Mice were immunised either once, with adjuvant as control (A) or DTV (D) only, or twice (i.e. with booster), using adjuvant alone followed by DTV at subsequent recapture ≥7 days after the previous immunisation (AD), DTV and subsequently adjuvant alone (DA), or two separate doses of DTV (DD). Only mice immunised at least 7 days prior to antibody measurement are shown, and each point represents one of 1–4 recaptures for each individual. \textbf{B}, distribution of posterior distributions of the coefficients for diet, habitat, and vaccine formulation (A, D, AD, DA, DD) of a multilevel Bayesian model adjusted for repeated individual measures. Each density curve represents the distribution of 3,000 samples from one of four MCMC chains (see Fig~\ref{supfig:igg1}).}
...
\newcommand{\FigDiet}{
    \begin{figure}[hp]
        ...
        \includegraphics[width=0.5\textwidth, keepaspectratio=true]{./Figures/plots/MultiLevel_NDE_D_E_chn.pdf}
...
\newcommand{\TblPrecis}{
    \begin{table}[htbp]
        \centering
        \caption{Posterior predictions of model parameters for random intercept Gaussian model \(E \sim Normal(\alpha[ID] + H + D + V, \sigma)\).}
        \sffamily
        \begin{tabular}{|>{\bfseries}l|ccccc|}
            \hline
            Parameter & \textbf{Mean} & \textbf{Std} & \textbf{5\%} & \textbf{50\%} & \textbf{95\%} \\
            \hline
            Intercept & -0.476        & 0.851        & -1.878       & -0.470        & 0.902         \\
            Diet      & -0.273        & 0.079        & -0.402       & -0.274        & -0.142        \\
            Habitat   & -0.704        & 0.087        & -0.850       & -0.703        & -0.560        \\
            A         & 0.105         & 0.838        & -1.257       & 0.100         & 1.480         \\
            D         & 1.391         & 0.836        & 0.027        & 1.385         & 2.770         \\
            AD        & 1.046         & 0.838        & -0.321       & 1.046         & 2.424         \\
            DA        & 1.655         & 0.839        & 0.288        & 1.652         & 3.037         \\
            DD        & 2.017         & 0.840        & 0.650        & 2.014         & 3.404         \\
            \hline
        \end{tabular}
        \label{tbl:precis}
    \end{table}
